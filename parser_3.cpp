...
{
    //fprintf(outbuffer, "[parser] d Feldstart %d: '%c' @ %ld\n", idx, *p, p - line);
...
inline void parse_field_ignore(char*& p, char* line, FILE* outbuffer) {
    fprintf(outbuffer, "[parser] ign. Feld: '%c' [%ld] -", *p, p - line);
...
// --- Hauptfunktion ---
extern "C" size_t parser_3(char* line, void* out) {
    FILE* outbuffer = fopen("parser_3_out.txt", "a");
    if (!outbuffer) outbuffer = stdout;
    char* p = line;
    uintptr_t* fields = (uintptr_t*)out;
    parse_field_d(p, fields, 0, line, outbuffer);
    parse_field_d(p, fields, 1, line, outbuffer);
